...
# LOAD DATA ============================================================================  
  ...
  site_attrb_tbl <- map_dfr(site_attrib_df, ~.x %>% 
        ...
        count(value, operatingunit) %>% 
        mutate(value = ifelse(str_detect(value, "Dispens"), 
                                 "Dispensary or Pharmacy", value))) %>% 
        arrange(value) %>% 
        spread(value, n)
...
# SUMMARY TABLE -----------------------------------------------------------
  ...
  site_attrb_tbl %>%
    ...
    gt_color_rows(columns = c(2:9), na.color = "white", 
    ...
    gtsave_extra(filename = glue("Images/OU_DAA_summary_by_facility_type.png"))
...
# VIZ ============================================================================
  ...
  si_save("Graphics/OU_DAA_summary_by_facility_type.svg")
